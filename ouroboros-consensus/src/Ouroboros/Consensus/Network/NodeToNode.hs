...
mkApps
  ...
  -> Handlers m remotePeer blk
  -> Apps m remotePeer bCS bBF bTX bKA NodeToNodeClientResult ()
mkApps kernel Tracers {..} mkCodecs ByteLimits {..} genChainSyncTimeout ReportPeerMetrics {..} Handlers {..} =
  ...
  where
    aChainSyncClient
      ...
      -> remotePeer
      -> Channel m bCS
      -> m (NodeToNodeClientResult, Maybe bCS)
              ...
              (r, trailing) <-
                runPipelinedPeerWithLimits
                  ...
                  $ hChainSyncClient them version controlMessageSTM
                      ...
                      varCandidate
              return (ChainSyncClientResult r, trailing)
    ...
    aBlockFetchClient
      ...
      -> remotePeer
      -> Channel m bBF
      -> m (NodeToNodeClientResult, Maybe bBF)
    aBlockFetchClient version controlMessageSTM them channel = do
      ...
      bracketFetchClient (getFetchClientRegistry kernel) version them $ \clientCtx -> do
        ...
        return (NoClientResult, trailing)
    ...
    aTxSubmission2Client
      ...
      -> remotePeer
      -> Channel m bTX
      -> m (NodeToNodeClientResult, Maybe bTX)
    ...
    aKeepAliveClient
      ...
      -> remotePeer
      -> Channel m bKA
      -> m (NodeToNodeClientResult, Maybe bKA)
